...
export default function ProductCard({ product, index }) {
  ...
  const whatsappMessage = `Hola! Me interesa el producto: ${product.name} (${product.brand})${sizeText}`
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-cream">
        ...
        {product.size && (
          <span className="absolute top-3 right-3 bg-gold/90 backdrop-blur-sm text-warm-white text-[10px] font-medium uppercase tracking-wider px-3 py-1 rounded-full">
            {product.size}
          </span>
        )}
      ...
      <div className="p-5">
        ...
        <div className="pt-3 border-t border-gold/10">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="w-full inline-flex items-center justify-center gap-2 bg-gold/10 hover:bg-gold text-gold hover:text-warm-white text-xs font-medium uppercase tracking-wider px-4 py-3 rounded-full transition-all duration-300 hover:shadow-md hover:shadow-gold/20"
          >
